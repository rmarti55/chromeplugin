...
- **Honest time.** Two clocks from the same event log: **Passive in Chrome** (browser on screen) and **Active in Chrome** (on screen + recent input). On Mac, **Passive on Mac** / **Active on Mac** for all apps. Site breakdowns use active time. See [`docs/time-model.md`](docs/time-model.md).
...
1. As you browse, the service worker records events (`activate`, `urlchange`, `focus`, `blur`,
   `idle`, `active`) to IndexedDB with timestamps. **Passive in Chrome** accrues while Chrome is the
   frontmost app; **Active in Chrome** pauses after ~5 min without input (Chrome **idle**) or when you switch apps.
