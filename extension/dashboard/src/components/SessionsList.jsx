    ...
    <div className="flex items-start justify-between text-sm gap-3 py-2.5 border-b border-stone-100 last:border-0">
      ...
      <div className="shrink-0 flex items-center gap-3 text-right tabular-nums">
        {!row.isNoiseRollup && row.mirrorVisits > 0 && (
          <span className="text-stone-400 text-xs">{row.mirrorVisits} navs</span>
        )}
  ...
  return (
    <div
      className={`flex items-start justify-between text-sm gap-3 py-2.5 border-b border-stone-100 last:border-0 ${
        highlight ? "bg-amber-50/50 -mx-1 px-1 rounded" : ""
      }`}
    ...
    >
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          <div className="font-medium text-stone-800 truncate">{label}</div>
          {row.isHistoryOnly && (
            <span className="shrink-0 text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-stone-100 text-stone-500 border border-stone-200">
              History only
            </span>
          )}
          {badge && (
            <span
              className="shrink-0 text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200"
          ...
          {flag && row.alignment !== "noise" && (
            <span className="shrink-0 text-[10px] text-amber-700">{flag}</span>
          )}
        </div>
        <div className="text-stone-500 text-xs truncate mt-0.5">
          {row.isNoiseRollup ? (
      ...
      <div className="shrink-0 flex items-center gap-3 text-right tabular-nums">
        ...
        {hasHistory ? (
          <>
            <span className="text-stone-500 text-xs w-14 hidden sm:block">
              {showOpen ? formatDuration(row.mirrorOpenSeconds) : "—"}
            </span>
            <span className="text-stone-400 text-xs w-14 hidden sm:block">
              {row.historyVisits || "—"}
            </span>
            <span className="text-stone-400 text-xs w-14 hidden sm:block">
      ...
      {hasHistory && (
        <details className="mt-4 group">
          ...
          {historyAlignment.summary && (
            <p className="text-sm text-stone-600 mt-3 p-3 rounded-lg bg-stone-50 border border-stone-200">
              {historyAlignment.summary}
              {historyAlignment.trend && (
                <span className="block text-xs text-stone-500 mt-1">{historyAlignment.trend}</span>
              )}
            </p>
          )}
          ...
          <div className="hidden sm:grid sm:grid-cols-[1fr_auto_auto_auto_auto] gap-x-3 text-[11px] tracking-wide text-stone-400 mb-2 px-1">
            <span>Site</span>
