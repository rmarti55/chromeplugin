...
function HistoryTable({ rows }) {
  if (!rows.length) {
    return <p className="text-sm text-stone-700">No History entries for this day.</p>;
  }
        ...
        <tbody className="divide-y divide-stone-100">
          {rows.map((row) => {
            ...
            return (
              <tr
                key={row.domain}
                className={highlight ? "bg-accent-soft/50" : undefined}
              ...
              >
                ...
                <td className="py-2 text-stone-600 text-right tabular-nums">
                  {row.mirrorVisits}
                </td>
              </tr>
            );
...
export function HistoryReferenceDetails({ alignment }) {
  ...
  return (
    <details className="mt-4 group">
      ...
      <p className="text-sm text-stone-600 mt-2 mb-1">
